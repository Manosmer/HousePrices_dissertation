...
It is, also, worth mentioning that $R_{(adj)}^2$, which takes the complexity of the used model into account, improves from the full model's value of `r 1 - ( full_model_RSS/(nrow(trainingSet)-18) )/( TSS/(nrow(trainingSet)-1) )` to `r 1 - ( selectedModel_RSS/(nrow(trainingSet)-8) )/( TSS/(nrow(trainingSet)-1) )`, suggesting that the predictors in model [4.2.1](#VSModel) are able to explain 86.23% of the variation in the sale price.
...
```{r, out.width=".49\\linewidth", fig.cap="\\label{fig:VSassumptions} Plots of the residuals from fitting model 4.2.1 on the training set.", fig.show='hold', fig.height=3.25, fig.width=3.25}
