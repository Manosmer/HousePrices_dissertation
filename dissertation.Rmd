...
# trainingSet <- trainingSet[!(rownames(trainingSet)=="32" | rownames(trainingSet)=="438"),]
...
As mentioned in book [ESLII], "Variable Selection" can be considered as a _discrete_ technique, in the sense that complexity reduction is achieved by choosing to either include a variable in the model or exclude it from it. 
...
The difference between the two regularised regression methods is in their penalty function. Ridge regression uses the squared $L_2-norm$ of the parameters' vector, while Lasso uses the $L_1-norm$. This small difference results in a different behaviour.

```{r}
theta <- seq(-4,4,length=100)
plot(c(-4,4),c(-1,17), type = "n", xlab=expression(theta), ylab="Penalty")
lines(theta,abs(theta), lty=1, col = 1)
lines(theta,theta^2, lty=2, col = 2)
legend("top", legend=c("Lasso", "Ridge"), col = c(1,2), lty=c(1,2))
```


Shrinkage, penalty, Bias

Application similar to the one described in book [An Introduction to Statistical Learning]
...
lambdas = seq(1000000, 0.01, length=8000)
X <- model.matrix(formula(price ~ bath*precip), data=trainingSet)[,-1]
cvfit <- cv.glmnet(X, trainingSet$price, alpha=0, nfolds=5, type.measure="mse", foldid=folds, lambda=lambdas)
cvfit$lambda.min
min(cvfit$cvm)
coef(cvfit)[,cvfit$lambda == cvfit$lambda.min]
```

```{r FindLambdaCV}
lambdas <- 10^seq(20, -2, length=10000)
findLambda_res <- RR_findLambdaCV(price ~ precip*bath, alpha=0, lambdas, trainingSet, splits)
findLambda_res$lambdas[which.min(findLambda_res$MSPE)]
min(findLambda_res$MSPE)

X <- model.matrix(formula(price ~ bath*precip), data=trainingSet)[,-1]
coef(glmnet(X, trainingSet$price, family="gaussian", alpha=0, lambda=10^20, maxit = 10^8, thresh=1e-12))
```


```{r, eval=TRUE}
#lambdas <- 10^seq(20, -3, length=8000)

lambdas <- 10^seq(25, -2, length=10000)
...
X <- model.matrix(formula(price ~ precip*bath + dist_am1*parking + sqft + elevation), data=trainingSet)[,-1]
coef(glmnet(X, trainingSet$price, family="gaussian", alpha=1, lambda=10^25, maxit = 10^8, thresh=1e-12))
