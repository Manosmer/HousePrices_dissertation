...
The results from fitting the above model on the training data set is shown in table \ref{tab:varSelModel}. The reported p-values indicate that, accounting for all of the variables we used in our model, the categorical variable _"bath"_ and the interaction between precipitaion and _"bath"_ seem to be significant predictors regarding the sale price of a house. More specifically, our fitted model estimates that:
...
It is, also, worth mentioning that $R_{(adj)}^2$, which takes the complexity of the used model into account, improves from the full model's value `r 1 - ( full_model_RSS/(nrow(trainingSet)-19) )/( TSS/(nrow(trainingSet)-1) )` to `r 1 - ( selectedModel_RSS/(nrow(trainingSet)-8) )/( TSS/(nrow(trainingSet)-1) )`, suggesting that we are able to explain 86.23% of the variation in the sale price.
...
```{r, out.width="48%"}
plot(selectedModel, which=c(1,2,5), sub.caption="")
ggplot(mapping=aes(x=selectedModel$residuals)) + 
  geom_histogram(bins=28) + xlab("Residuals")
# bins 28
```
...
variable selection: discrete. Shrinkage methods are continuous allowing for more flexible complexity reduction.
~~ TODO ~~
