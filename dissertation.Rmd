...
$$\beta_7\cdot{sqft_i} + \beta_8\cdot{precip_i} + \beta_9\cdot{elev_i} + \beta_{10}\cdot{dist1_i} + \beta_{11}\cdot{dist2_i} + \beta_{12}\cdot{dist3_i} + \epsilon_i$$
$$,\epsilon_i\sim N(0, \sigma^2)$$
, where the indicator variables are equal to $1$ when the $i^{th}$ observation's corresponding variable takes the related value and $0$ otherwise. (**Baseline categories**: bath=1, parking="Covered")

```{r}
fullModel <- lm(price ~ ., data = houseprices)
get_regression_table(fullModel) %>%
  select(term, estimate, p_value, lower_ci, upper_ci) %>%
  kable(caption = "\\label{tab:FullModelFit} The estimated regression coefficients along with the corresponding p-values and the Confidence Intervals from fitting the full additive model.")
```
