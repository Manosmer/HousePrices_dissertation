...
However, the most important thing to note here is the mean squared prediction error on the validation set, which is equal to $\hat{MSPE}=$ `r sum( (validationSet$price - predict(fullModel, validationSet))^2 )/nrow(validationSet)`. This metric will be used for variable selection as described in the following subsection.
