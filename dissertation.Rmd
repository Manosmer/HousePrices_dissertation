...
A more detailed investigation on multicollinearity can be conducted by fitting linear regression models using each one of the distance variables as the response each time. The values of $R_{(adj)}^2$ reveal that we can explain 72.06% of the variability in _"dist\_am3"_ using the other two variables. Moreover, when we treat any of the other two variables as our response and modelling it with the other two, _"dist\_am3"_ is almost solely responsible for observing a high $R_{(adj)}^2$ value. In fact, when we use just _"dist\_am2"_ to model _"dist\_am1"_ we can explain just 19.09% of its variability in contrast to 63.16% when we use just _"dist\_am3"_.

In terms of _"price"_, the $R^2$ score from using just _"dist\_am3"_, although very small, is better than when using _"dist\_am1"_ and _"dist\_am2"_, together or alone. Interestingly though, the p-value for the coefficient estimate of _"dist\_am3"_ is smaller than the 5% significance level we chose to use in this paper. This suggests that there is a statistically significant relationship between _"dist\_am3"_ and our response variable, however, _"dist\_am3"_ accounts for a small percentage of the total variability in _"price"_.

Finally we note that, before we decide on removing any of the distance variables in order to overcome multicollinearity, it is important to assess their relationship to _"price"_ through their interactions with the categorical variables in the data. The exploration of interactions is presented in a following subsection.
...
```{r}
fullmodel_variables <- c("bath", "parking", "precip*bath", "dist_am3*parking", "sqft", "elevation")
...
RR_findLambdaCV <- function(f, alpha, lambdas, dataset, splits) {
  ...
  for(i in 1:k) {
    ...
    MSPEs_perLambda <- MSPEs_perLambda + mean( (validSet[[responseVar]] - predict(ridgeModel, model.matrix(f, data=validSet)[,-1], exact=TRUE))^2)
  }
...
predict(glmnet(X, trainingSet$price, alpha=1, lambda=lambdaGrid, maxit=10^8, thresh=1e-12), type="coefficients",s= cvfitLasso$lambda.min, exact=TRUE)

predict(glmnet(X, trainingSet$price, alpha=1, lambda=lambdaGrid, maxit=10^8, thresh=1e-12), type="coefficients",s= cvfitLasso$lambda.1se, exact=TRUE)
```



```{r, fig.align='center', out.width="0.49\\linewidth", fig.show='hold', fig.cap="\\label{fig:chooseLambda} The change in the estimated MSPE as we increase the regularisation parameter, $\\lambda$. The dashed vertical line corresponds to the chosen $\\lambda$ for each regularised regression method, that yields the best CV MSPE."}
plot(c(10^-3, 400), c(min(cvfitRidge$cvm)-10^5, max(cvfitRidge$cvm[cvfitRidge$lambda <= 400])), type="n", ylab="Cross-validation MSPE", xlab=expression(lambda), main="Ridge on the best-subset model")
...
We tried Ridge on the Lasso variable set with worse results than best-subset. We have also applied ridge straight to the initial model without results. Change lambda grid from 20000 to 40000
